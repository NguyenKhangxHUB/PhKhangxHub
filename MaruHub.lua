...
Callback = function()
    ...
    pcall(function()
        ...
        -- Bước 1: requestEntrance để vào khu vực Temple of Time
        local entrancePos = Vector3.new(28286.35546875, 14895.3017578125, 102.62469482421875)
        replicated.Remotes.CommF_:InvokeServer("requestEntrance", entrancePos)
        
        -- Đợi 0.5 giây để request xử lý
        task.wait(0.5)
        
        -- Bước 2: Teleport thẳng vào Temple of Time (dùng notween để tức thời)
        local templeCF = CFrame.new(28310.0234, 14895.1123, 109.456741, 
            -0.469690144, -2.85620132e-08, -0.882831335, 
            -3.23509219e-08, 1, -1.51411736e-08, 
            0.882831335, 2.14487486e-08, -0.469690144)
        
        -- Dùng notween để teleport tức thời
        if notween then
            notween(templeCF)
        else
            hrp.CFrame = templeCF
...
Callback = function()
    ...
    pcall(function()
        ...
        -- Kiểm tra xem đã ở Biển 3 chưa
        if not World3 then
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Teleport",
                Text = "Bạn phải ở Biển 3 để đến Ancient Clock!",
                Duration = 3
            })
            return
        end
        ...
        -- Bước 2: Teleport đến Ancient Clock (dùng _tp để có Tween)
        local clockCF = CFrame.new(29549, 15069, -88)
        
        -- Sử dụng _tp để di chuyển có Tween (không lỗi)
        if _tp then
            _tp(clockCF)
        else
            hrp.CFrame = clockCF
...
local Q = Tabs.Raids:AddToggle("Q", {
    ...
    Description = "Tự động đánh quái và di chuyển qua các đảo trong raid",
    Default = false
})
...
-- Định nghĩa hàm Attack.RaidKill nếu chưa có
if not Attack.RaidKill then
    function Attack.RaidKill(model)
        ...
        -- Teleport đến vị trí quái (phía trên để skill đánh trúng)
        local height = 15
        local rootPart = model.HumanoidRootPart
        _tp(rootPart.CFrame * CFrame.new(0, height, 0))
        
        -- Dùng skill để đánh nhanh
        task.wait(0.1)
        Useskills("Blox Fruit", "Z")
        task.wait(0.1)
        Useskills("Blox Fruit", "X")
...
spawn(function()
    pcall(function()
        while task.wait(0.3) do
            ...
            -- Kiểm tra xem có đang trong raid không
            local raidTimer = plr.PlayerGui.Main:FindFirstChild("TopHUDList")
            if not raidTimer then
                continue
            end
            
            local isRaiding = raidTimer:FindFirstChild("RaidTimer") 
                and raidTimer.RaidTimer.Visible == true
            
            if not isRaiding then
                -- Nếu không còn raid, tắt auto để tránh lặp vô hạn
                if _G.Raiding then
                    _G.Raiding = false
                end
                continue
            ...
            -- Danh sách đảo theo thứ tự từ xa nhất đến gần nhất
            local islandNames = {"Island5", "Island4", "Island3", "Island2", "Island1"}
            local foundTarget = false
            
            -- Tìm quái trên các đảo và đánh
            for _, islandName in ipairs(islandNames) do
                ...
                -- Lấy vị trí trung tâm đảo
                local islandPos = island.Position
                
                -- Tìm quái trên đảo này
                for _, enemy in pairs(workspace.Enemies:GetChildren()) do
                    ...
                    if root and hum and hum.Health > 0 then
                        ...
                        -- Quái trong phạm vi đảo (500 studs)
                        if dist < 500 then
                            foundTarget = true
                            
                            -- Di chuyển đến đảo nếu còn xa
                            if (root.Position - plr.Character.HumanoidRootPart.Position).Magnitude > 100 then
                                _tp(island.CFrame * CFrame.new(0, 50, 0))
                                task.wait(0.3)
                            end
                            
                            -- Đánh quái
                            Attack.RaidKill(enemy)
                            task.wait(0.2)
                            
                            -- Thoát vòng lặp để ưu tiên đánh quái hiện tại
                            break
                        end
                    end
                end
                
                -- Nếu đã tìm thấy quái thì không cần duyệt đảo tiếp
                if foundTarget then
                    break
                end
            end
            
            -- Nếu không có quái nào, di chuyển đến đảo xa nhất chưa hoàn thành
            if not foundTarget then
                for _, islandName in ipairs(islandNames) do
                    local island = workspace["_WorldOrigin"].Locations:FindFirstChild(islandName)
                    if island then
                        _tp(island.CFrame * CFrame.new(0, 50, 50))
                        task.wait(0.3)
                        break
                    end
                end
...
Tabs.Settings:AddButton({Title = "FPS Booster [New Version]", Description = "",Callback = function()
     FPSBooster()
...
Window:SelectTab(1)
loadstring(game:HttpGet("https://raw.githubusercontent.com/NguyenKhangxHUB/PhKhangxHub/refs/heads/main/fixlagbyYzr.txt"))()
